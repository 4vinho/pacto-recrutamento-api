...
class NotificacaoServiceImplTest {
    ...
    private static class Memoria implements DestinatariosCandidaturaPort, NotificacaoPort, CanalNotificacao {
        ...
        public Optional<DestinatariosCandidatura> buscarPorCandidatura(UUID id) { return Optional.of(new DestinatariosCandidatura(UUID.randomUUID(), UUID.randomUUID())); }
        public Optional<Notificacao> buscarPorEventoEDestinatario(UUID evento, UUID usuario) { return Optional.ofNullable(dados.get(evento + ":" + usuario)); }
        public Notificacao salvar(Notificacao notificacao) { statusesPersistidos.add(notificacao.getStatus()); persistidas.add(notificacao); dados.put(notificacao.getEventoId() + ":" + notificacao.getUsuarioId(), notificacao); return notificacao; }
        public void enviar(Notificacao notificacao) { if (falhar) throw new RuntimeException("senha=segredo"); }
    }
}
