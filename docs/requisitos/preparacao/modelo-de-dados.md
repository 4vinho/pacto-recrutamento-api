...
Somente um currículo pode estar ativo por candidato. Essa regra deve ser garantida por índice único parcial considerando registros cujo `excluido_em` seja nulo.

O arquivo não deve ser armazenado no PostgreSQL. O campo `storage_key` referencia o objeto mantido no bucket privado `curriculos`.
...
Critérios estruturados, como tempo mínimo de empresa, devem ser adicionados somente quando as regras de avaliação forem definidas.
...
Os campos devem acompanhar os equivalentes de vaga, pergunta e requisito. Ao criar uma vaga a partir de um template, seus dados devem ser copiados para preservar o histórico.
